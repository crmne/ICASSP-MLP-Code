...
def read_wav(filename):
    bits_per_sample = '16'
    filename = "\"" + filename + "\""
    cmd = ['sox', filename, '-t', 'raw', '-e', 'unsigned-integer', '-L', '-c', '1', '-b',
           bits_per_sample, '-', 'pad', '0', '30.0', 'rate', '22050.0', 'trim', '0', '30.0']
    cmd = ' '.join(cmd)
    print cmd
    raw_audio = numpy.fromstring(subprocess.Popen(
        cmd, stdout=subprocess.PIPE, shell=True).communicate()[0], dtype='uint16')
...
class FeatExtraction():
    ...
    def get_filenames(self,):
        ...
        self.filenames = [l.strip()
                          for l in open(dataset_files, 'r').readlines()]
    ...
    def setup_h5(self,):
        ...
        self.h5_x = self.h5.createEArray('/', 'x', tables.FloatAtom(
            itemsize=4), self.x_earray_shape, chunkshape=self.chunkshape, expectedrows=self.num_files)
        self.h5_filenames = self.h5.createEArray(
            '/', 'filenames', tables.StringAtom(256), (0,), expectedrows=self.num_files)

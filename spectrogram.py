...
class SpecGram():
    ...
    def __init__(self, x, fs, winSize, nOverlaps=2, logAmplitude=False, do_compress=True, window_type='square'):
        ...
        # self.numWindows=(int(len(x)/self.winSize)-1)*self.nOverlaps+1 #This
        # always gave odd number of windows
        self.numWindows = int(
            (len(x) - (self.winSize - self.overLen)) / self.overLen)  # 1290

        # the first and middle value of the fft are not symetric
        self.fftLen = self.winSize / 2 + 1
        self.nyquist = self.fs / 2
        self.freqs = 1.0 * \
            N.arange(self.fftLen) / (self.fftLen - 1) * self.nyquist
        ...
        for i in range(self.numWindows):
            ...
            specData = N.fft.fft(
                x[i * self.overLen:i * self.overLen + self.winSize] *
                self.window)
